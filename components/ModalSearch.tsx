...
export const ModalSearch = ({
    ...
    const handleSelectOption = (producto: any) => {
        onSelectItem(producto.id as string)
        onClose()
    }
    ...
    const handleInputChange = async (event: any) => {
        ...
        const data = await onInputChange(term)
                ...
                <div className={styles.resultsSearch}>
                    {
                        (inputValue !== "" && searchResults?.length > 0) ?
                            searchResults.slice(0, 10)?.map((producto: any, index: number) =>

                                <SearchItemCard
                                    key={index}
                                    productName={producto?.name ? producto?.name as string : producto as string}
